...
## M-J10 增补：可选加密（`src/lib/backup-crypto.ts`）

明文 `.aiwx` 里躺着**全部隐藏私信**（AI↔AI 会话）、全部记忆事实、全部朋友圈。
这个 App 花了很大力气让隐藏会话不泄漏到任何用户可见面（`search()` 内部过滤、
收藏页、通知、导出预览逐个审过），而导出一份明文 JSON 到下载目录，一次就把
那些努力全部绕过去了。

### 格式

信封本身是**明文 JSON**，`parseBackup` 之前就能认出来：

```
{ aiwx: 'enc', v: 1, kdf: 'PBKDF2-SHA256', iter, salt, iv, ct }
```

PBKDF2-SHA256 十万轮 → AES-GCM-256。**`iter` 从文件里读**，不是用当前常量——
把参数写死等于每次调参作废一批旧备份。

### 三条不许动的规则

1. **口令错 = 抛 `BadPassphraseError`，绝不返回半截明文。** 恢复流程拿到字符串
   就会去 `parseBackup`，一个「像 JSON 的垃圾」比一次干净的失败危险得多——
   它会覆盖用户的真数据。GCM 的 tag 认证失败与被篡改无法区分，也不必区分。
2. **与 `keystore.ts` 没有关系，而且必须没有。** keystore 用一把
   `extractable:false` 的设备本机主密钥封 API key，那把钥匙的全部价值就在于
   出不去；这里的密钥从用户口令现场派生、用完就扔。共用一把会立刻毁掉前者
   （备份要能在另一台设备上打开＝密钥必须可导出）。
3. **加密不是把 `__crypto_master` 带上的借口。** 行级排除仍由
   `isPortableSettingRow` 完成，与加不加密无关——带上了也没用：另一台设备的
   IndexedDB 里放一把别人的 CryptoKey 只会让那台机器的 keystore 永久解不开。

### 只加密离开设备的那一份

设备上的**备份历史**保持明文：它躺在 App 自己的存储里，和数据库同一层保护，
给它上锁只会制造一种新的失败——用户忘了口令，连自己机器上的历史都恢复不了。
加密要挡的是「.aiwx 被复制走」，不是「有人拿到了这台已解锁的手机」。

导出时口令要输**两遍**：忘记口令的备份是一块砖，而备份是这个 App 里用户唯一的
数据保险。开关默认关。
