...
import { explainDrift, type DriftExplanation } from '../../ai/drift';
...
export function StatusPage() {
  ...
  // The affect pulse is the one stored input; read it once per minute tick.
  ...
  const drift = explainDrift(contactId, now, epoch);
      ...
      <div className="page-body status">
        <div className="status__head">
          ...
          <div className="status__head-id">
            ...
            <div className="status__summary">{drift.summary}</div>
          </div>
        </div>
...
function DriftCard({ drift }: { drift: DriftExplanation }) {
  return (
    <div className="status__card">
      <div className="status__card-title">最近的变化</div>
      {drift.dims.map((d) => (
        <div key={d.key} className="status__dim">
          <CenteredMeter label={d.label} value={d.value} left="−" right="+" />
          <div className="status__dim-reason">{d.reason}</div>
        </div>
      ))}
    </div>
  );
