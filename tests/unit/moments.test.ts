...
describe('planReactions', () => {
  ...
  it('is deterministic for the same moment and seed', () => {
    const one = planReactions('m1', 'self', NOON, crowd, 's');
    const two = planReactions('m1', 'self', NOON, crowd, 's');
    expect(one).toEqual(two);
  });

  it('gives different moments different crowds', () => {
    const a = planReactions('m1', 'self', NOON, crowd, 's');
    const b = planReactions('m2', 'self', NOON, crowd, 's');
    expect(a).not.toEqual(b);
  });

  it('never lets the author react to their own post', () => {
    const planned = planReactions('m1', 'a', NOON, crowd, 's');
    expect(planned.every((p) => p.contactId !== 'a')).toBe(true);
  });

  it('returns reactions sorted by time', () => {
    const planned = planReactions('m1', 'self', NOON, crowd, 's');
  ...
  it('puts a commenter’s comment after their own like', () => {
    ...
    const planned = planReactions('m1', 'self', NOON, certain, 's');
  ...
  it('rate 0 means nobody reacts; rate 1 means everybody does', () => {
    ...
    expect(planReactions('m1', 'self', NOON, none, 's')).toEqual([]);

    const all = crowd.map((r) => ({ ...r, likeRate: 1, commentRate: 1 }));
    const planned = planReactions('m1', 'self', NOON, all, 's');
  ...
  it('higher affinity draws more reactions than lower affinity', () => {
    ...
    const cold = planReactions('m1', 'self', NOON, big.map((r) => ({ ...r, affinity: 0 })), 's');
    const warm = planReactions('m1', 'self', NOON, big.map((r) => ({ ...r, affinity: 100 })), 's');
  ...
  it('lands every reaction inside the reactor’s active hours', () => {
    ...
    const planned = planReactions('m1', 'self', NOON, narrow, 's');
